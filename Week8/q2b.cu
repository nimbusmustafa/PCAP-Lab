...
__global__ void matrixMultiplyColumn(int *A, int *B, int *C, int M1, int K1) {
    int col = blockIdx.x;  // Each thread handles one column of C
    if (col < N) {
        for (int i = 0; i < M1; i++) {
            C[i * N + col] = 0;
            for (int k = 0; k < K1; k++) {
                C[i * N + col] += A[i * K1 + k] * B[k * N + col];
            }
        }
    }
}
...
int main() {
    ...
    matrixMultiplyColumn<<<N, 1>>>(d_A, d_B, d_C, M, K);
